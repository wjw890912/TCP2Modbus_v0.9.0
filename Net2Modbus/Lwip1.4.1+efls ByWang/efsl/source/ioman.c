...
esint8 ioman_init(IOManager *ioman, hwInterface *iface, euint8* bufferarea)
{
	...
	ioman->iface=iface;
	//IO取得缓存内的buf地址，这个是个数组长度是  cache_mem[IOMAN_NUMBUFFER * 512];
	//如果bufferarea不为0表示用户提供的内存，使用之，反之使用文件系统提供的告诉缓存地址
	ioman->bufptr = ioman_getBuffer(ioman,bufferarea);
	ioman->numbuf = IOMAN_NUMBUFFER;//多少个512	 默认1
	ioman->numit  = IOMAN_NUMITERATIONS;//迭代次数	 默认1
	
	ioman_reset(ioman);//清空IOman结构
	return(0);
}
/*****************************************************************************/
...
void ioman_reset(IOManager *ioman)
{
	...
	memClr(ioman->sector,sizeof(euint32)*ioman->numbuf);//输入输出管理的扇区清空
	memClr(ioman->status,sizeof(euint8) *ioman->numbuf);//状态清空
	memClr(ioman->usage ,sizeof(euint8) *ioman->numbuf);//使用状态清空
	memClr(ioman->itptr ,sizeof(euint8) *ioman->numbuf);//itptr 清空
	ioman_setError(ioman,IOMAN_NOERROR);//设置错误为没有错误
			//遍历ioman->stack栈并清空，是个二维数组 ioman->stack[1][1]
	for(nb=0;nb<ioman->numbuf;nb++){
		for(ni=0;ni<ioman->numit;ni++){
			ioman->stack[nb][ni].sector=0;//扇区
			ioman->stack[nb][ni].status=0;//状态
			ioman->stack[nb][ni].usage =0;//使用
		}
	}
}
/*****************************************************************************/
...
euint8* ioman_getPtr(IOManager *ioman,euint16 bufplace)
{
	...
	/*返回高速缓存+bufplace*512，这个bufplace是高速缓存的序号，
	代表数组的，比如0就是首地址1就是512地址。由于是在首地址偏移的所以要加上*/
}
/*****************************************************************************/
...
{
	euint16 c;
	//取出(ioman->status[bufplace]&(1<<attribute));attribute=0；
	for(c=0;c<ioman->numbuf;c++){
		if(ioman_isValid(c) && ioman->sector[c] == address)return(c);
		//地址和当前IOMAN管理的扇区匹配且status的最低位有效。则返回C是个索引，高速缓存的序号
	}
	return(-1);
}
/*****************************************************************************/
